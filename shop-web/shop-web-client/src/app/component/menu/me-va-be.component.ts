...
@Component({
  ...
  templateUrl: '../../view/menu/me-va-be.html'
})

export class MeVaBeComponent implements AfterViewInit {
  ...
  ngAfterViewInit(): void {
    this.homePageService.getAdsImageProductGroupIds(8).then(result => {
      this.adsImageHeaderIds = result.data;
    });
    this.productGroupService.getMenuGroup("mevabe").then(result => {
  ...
  onLoadPage(page: number) {
    this.productGroupService.getProductByMenu("mevabe", page, 30).then(result => {
