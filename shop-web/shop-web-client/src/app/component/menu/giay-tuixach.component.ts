 ...
 */
@Component({
  selector : 'giay-tuixach',
  styleUrls: ['../../style/group-product.css'],
  templateUrl: '../../view/menu/giay-tuixach.html'
})

export class GiayTuixachComponent implements AfterViewInit {
  ...
  ngAfterViewInit(): void {
    this.homePageService.getAdsImageProductGroupIds(6).then(result => {
      this.adsImageHeaderIds = result.data;
    });
    this.productGroupService.getMenuGroup("giaytuixach").then(result => {
  ...
  onLoadPage(page: number) {
    this.productGroupService.getProductByMenu("giaytuixach", page, 30).then(result => {
