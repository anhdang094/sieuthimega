 ...
 */
@Component({
  selector : 'noithat-giadung',
  styleUrls: ['../../style/group-product.css'],
  templateUrl: '../../view/menu/noithat-giadung.html'
})

export class NoithatGiadungComponent implements AfterViewInit {
  ...
  ngAfterViewInit(): void {
    this.homePageService.getAdsImageProductGroupIds(10).then(result => {
      this.adsImageHeaderIds = result.data;
    });
    this.productGroupService.getMenuGroup("noithatgiadung").then(result => {
  ...
  onLoadPage(page: number) {
    this.productGroupService.getProductByMenu("noithatgiadung", page, 30).then(result => {
